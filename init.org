...
#+begin_src elisp
    (user-message
      '("Any time you install a package with M-x package-list-packages and similar, pallet"
        "will write it down in your Cask file and the files fill go under the .cask/"
        "directory."))
    (require 'pallet)
    (pallet-mode t)
#+end_src
...
#+begin_src elisp
    (user-message
      '("Else, <tab> would not work with Magit."))
    (require 'magit)
...
#+begin_src elisp
  ...
  (user-message
    '("Collect items to be displayed in an agenda view from files listed in `org-agenda-files'"))
...
#+name: utf8
#+begin_src elisp
  (user-message '("Just make everything utf8."))
